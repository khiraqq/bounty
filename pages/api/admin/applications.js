...
export default async function handler(req, res) {
  ...
  // ── POST /api/admin/applications?action=review|approve|reject ────────────
  if (req.method === 'POST') {
    const admin = await assertAdmin(req, res);
    if (!admin) return;

    if (!['review', 'approve', 'reject'].includes(action)) {
      return res.status(400).json({ message: 'Invalid action. Use: review, approve, reject' });
    }
    ...
    try {
      ...
      if (action === 'review') {
        // Mark as reviewing — sends "In Review" message
        ...
        return res.status(200).json({ message: 'Application marked as under review. Message sent to applicant.' });
      }
      ...
      if (action === 'reject') {
        ...
        return res.status(200).json({ message: 'Application rejected. Message sent to applicant.' });
      }
    } catch (err) {
